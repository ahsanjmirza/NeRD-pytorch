...
def make_dataset(basedir, factor=16, recenter=True, bd_factor=0.75, spherify=False, path_zflat=False):
    """
    Create a dataset from the provided data directory.

    Args:
        basedir (str): The base directory containing the data.
        factor (int, optional): The factor to downsample the images by. Defaults to 16.
        recenter (bool, optional): Whether to recenter the images. Defaults to True.
        bd_factor (float, optional): The factor to scale the bounding box by. Defaults to 0.75.
        spherify (bool, optional): Whether to spherify the images. Defaults to False.
        path_zflat (bool, optional): Whether to flatten the z-axis of the camera path. Defaults to False.

    Returns:
        None
    """
    images, masks, _, poses, bds, render_poses = load_llff_data(
        basedir=basedir, 
        factor=factor, 
        recenter=recenter, 
        bd_factor=bd_factor, 
    ...
    fig.set_ylabel('Y')
    fig.set_zlabel('Z')
    plt.savefig(os.path.join(basedir, "poses_plot.png"))
    plt.clf()
    plt.cla()
    plt.close()

    pth = os.path.join(basedir, "data_all_"+str(factor))
    ...
    for i in range(images.shape[0]):
        data_dict = {
            ...
            "near": bds[i, 0], 
            "far": bds[i, 1],
            "near_global": np.floor(near_global),
            "far_global": np.ceil(far_global)
        }
        np.savez(os.path.join(pth, str(i)+".npz"), **data_dict)
